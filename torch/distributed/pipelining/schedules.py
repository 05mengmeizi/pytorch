...
def _determine_and_set_global_inference_mode(
    ...
    """
    Determine the global inference mode via collective across all PP ranks.
    If ANY stage (locally or remotely) needs DYNAMIC, all stages use DYNAMIC.
    """
    needs_dynamic = any(
        InferenceMode.needs_dynamic(stage._init_meta, has_backward) for stage in stages
    )
    ...
    for stage in stages:
        ...
        if global_mode == InferenceMode.STATIC:
            stage._stage_meta = stage._init_meta
        else:
            if stage._init_meta.has_any():
                stage._user_meta = stage._init_meta
    ...
    def _initialize_stage(self, args, kwargs, target=None):
        if not self._stage_forward_initialized:
            ...
            _determine_and_set_global_inference_mode(
                cast(list[PipelineStage], [self._stage]), self._has_backward
            )
