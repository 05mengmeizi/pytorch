...
class PallasKernel(SIMDKernel):
    ...
    def _detect_indirect_access(
        ...
        if len(buf_size_ints) < 2 or any(s is None for s in buf_size_ints):
            return None
        ...
        if indices_graph_name is not None:
            ...
            if indices_info is not None:
                ...
                indices_numel = math.prod(
                    v for s in indices_size
                    if (v := self._safe_int(s)) is not None
                )
                iter_product = math.prod(
                    length
                    for var in self._get_used_iter_vars(index)
                    if var in self.range_tree_nodes
                    if (length := self._safe_int(
                        self.range_tree_nodes[var].length
                    )) is not None
                )
    ...
    def _eliminate_dead_indirect_code(self) -> None:
        ...
        assignments: list[tuple[str | None, str, object]] = []
        for line in self.compute._lines:
            line_str = str(line).lstrip()
        ...
        self.compute._lines = [
            line for var_name, _, line in assignments
    ...
    def codegen_kernel(self, name: str | None = None) -> str:  # type: ignore[override]
        ...
        if use_scalar_prefetch:
            ...
            self._codegen_scalar_prefetch_wrapper(
                ctx, kernel_name, kernel_body_sp,
            )
    ...
    def _codegen_scalar_prefetch_wrapper(
        ...
        alias_set = set(ctx.alias_params)
        other_input_params = [
            p for p in ctx.kernel_input_params
        ...
        code.writeline(
            f"def {kernel_name}_kernel"
            f"({', '.join(prefetch_kernel_params)}):"
        )
        with code.indent():
        ...
        wrapper_params = (
            ...
            + ctx.size_var_params
            + ctx.kernel_input_params
        )
        static_argnums = list(range(2 + len(ctx.size_var_params)))
        static_argnums_literal = (
            "(" + ", ".join(str(x) for x in static_argnums) + ",)"
        )
        code.writeline(
        ...
        with code.indent():
            ...
            if ind_dim == 0:
                code.writeline(
                    f"_table_3d = {table}.reshape("
                    f"{table}.shape[0], 1, _D)"
                )
            else:
            ...
            code.writeline(
                "return tuple(r.reshape(s)"
                " for r, s in zip(_result, out_shapes))"
            )
