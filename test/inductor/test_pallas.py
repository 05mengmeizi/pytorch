...
class PallasTestsMixin:
    ...
    def _run_transformer(
        self, num_layers, seq_len, hidden_dim, num_heads, head_dim, ffn_dim,
        ...
        def transformer(x, mask, *layer_params):
            T, C = x.shape
            params_per_layer = 9  # rms_w1, rms_w2, w_q, w_k, w_v, w_o, w_gate, w_up, w_down
    ...
    def test_transformer_tiny(self):
        ...
        self._run_transformer(
            num_layers=4, seq_len=32, hidden_dim=64, num_heads=2, head_dim=32,
            ffn_dim=256,
        )
    ...
    def test_transformer_medium(self):
        ...
        self._run_transformer(
            num_layers=4, seq_len=128, hidden_dim=4096, num_heads=32, head_dim=128,
            ffn_dim=11008, atol=5e-3, rtol=1e-3,
        )

    def _run_transformer_lm(
        self, num_layers, seq_len, vocab_size, hidden_dim, num_heads, head_dim,
        ffn_dim, atol=1e-5, rtol=1.3e-6,
    ):
        """Run a full Llama-style LM (embedding + layers + norm + lm_head)."""
        torch._dynamo.reset()

        def transformer_lm(token_ids, embed_table, final_rms_w, lm_head_w, mask, *layer_params):
        ...
        result = compiled(token_ids, embed_table, final_rms_w, lm_head_w, mask, *all_params)
        expected = transformer_lm(token_ids, embed_table, final_rms_w, lm_head_w, mask, *all_params)
        self.assertEqual(result, expected, atol=atol, rtol=rtol)
    ...
    def test_transformer_lm_tiny(self):
        ...
        self._run_transformer_lm(
            num_layers=4, seq_len=32, vocab_size=256, hidden_dim=64,
            num_heads=2, head_dim=32, ffn_dim=256,
        )
